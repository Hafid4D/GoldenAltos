...
Alias project affectation.task.lot.phase.project

local Function get nameInWindowTitle()->$nameInWindowTitle : Text
	$nameInWindowTitle:=This:C1470.affectation.task.name
...
local Function isLast()->$boolean : Boolean
	
	$task:=This:C1470.affectation.task
	$boolean:=True:C214
	For each ($affectation; $task.affectations)
		For each ($taskTime; $affectation.taskTimes)
			If (This:C1470.stmpStart<$taskTime.stmpStart)
				$boolean:=False:C215
			End if 
		End for each 
	End for each 
