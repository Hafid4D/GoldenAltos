		...
		For each ($cip_e; $cip_es)
			...
			For each ($headerName; $headers)
				...
				Case of 
					...
					: ($headerName="priority")
						//$origin:=ds.CIOrigin.query("originID=:1"; $cip_e["origin"]).first()
					...
					: ($headerName="humanFactor")
						//$humanFactor:=ds.CIHumanFactor.query("factorID=:1"; $cip_e["humanFactor"]).first()
					...
					: ($headerName="disposition")
						...
						Else 
							...
							//$disposition:=ds.CIDisposition.query("dispositionID=:1"; $cip_e["disposition"]).first()
					...
					: ($headerName="category")
						//$category:=ds.CICategory.query("categoryID=:1"; $cip_e["category"]).first()
					...
					: ($headerName="IsAcceptable")
						//$IsAcceptable:=ds.YesNoQuestion.query("responseID=:1"; $cip_e["IsAcceptable"]).first()
