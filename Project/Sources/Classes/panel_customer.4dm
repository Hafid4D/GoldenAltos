...
Function bActionContact()
	//$refMenu:=Create menu
	//APPEND MENU ITEM($refMenu; "Open in new window"; *)
	//SET MENU ITEM PARAMETER($refMenu; -1; "openInWindow")
	
	//Case of 
	//: (Form.apContact=1)
	//$type:="AP"
	//: (Form.statusContact=1)
	//$type:="Status"
	//End case 
	
	//If (ds.Contact.query("UUID_Company = :1"; Form.current_item.UUID).query("title=:1"; $type).first()=Null)
	//DISABLE MENU ITEM($refMenu; -1)
	//End if 
	
	//$choice:=Dynamic pop up menu($refMenu)
	//RELEASE MENU($refMenu)
	//Case of 
	//: ($choice="openInWindow")
	//Form.sfw.openInANewWindow(ds.Contact.query("UUID_Company = :1"; Form.current_item.UUID).query("title=:1"; $type).first(); "customerService"; "contact")
	//End case 
	//This.LoadContact()
